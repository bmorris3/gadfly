...
default_hyperparameter_path = os.path.join(
    dirname, 'data', 'hyperparameters_granulation.json' #'hyperparameters.json'
)
...
class Hyperparameters(list):
    ...
    def for_star(
        ...
        scaled_delta_nu = scale._solar_delta_nu * scale.delta_nu(mass, radius)
        ...
        scaled_hyperparameters.extend(
            scale.p_mode_terms(
                scaled_nu_max, scaled_delta_nu,
                mass, radius, temperature, filt,
                ell_max=2, n_kernel_terms=400, n_max=30,
                solar_granulation_hyperparams=granulation_hyperparams,
                scaled_granulation_hyperparams=scaled_hyperparameters
            )
        )
    ...
    def __init__(self, identifier_or_filter, download=False):
        ...
        if isinstance(identifier_or_filter, str) and identifier_or_filter.upper() == 'SOHO VIRGO':
            ...
            if isinstance(identifier_or_filter, (tynt.Filter, Filter)):
                ...
                super().__init__(identifier_or_filter.wavelength, identifier_or_filter.transmittance)
            else:
